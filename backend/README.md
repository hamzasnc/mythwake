...
- Campaign, dungeon and tower combat accept an optional JSON body `{"heroIds":["hero_kael","hero_astra"]}`. The ordered selection must contain 1–7 distinct owned heroes; an omitted selection preserves the legacy seven-hero formation. Combat stats include selected heroes and shared bonuses once, without changing account-wide stats. Reuse the exact body and idempotency key for a retry.
- Combat replay `timeMs` is authoritative resolution time. Kael emits a zero-damage `action_start` record (`contactIndex: -1`, no `contactId`), followed by the action's contact records sharing `actionId` and `actionStartMs`. Each contact has a distinct `contactId`, a zero-based `contactIndex`, `contactCount` and a `contactTimeMs` offset from action start; `animationVariant` and `actionDurationMs` describe the animation. Process each ordered record once, and never apply backend damage from animation callbacks. All resolved events are returned without a count cap; pending contacts are absent when combat ends first. Kael runs on a 10 ms simulation grid while other heroes retain their existing 100 ms scheduling. HP remains one shared team pool and one enemy pool.
- Kael (`hero_kael`) is a free level-1 starter and a standard shard-pool entry. His single basic attack cycles through three 840 ms choreographies: `attack_cross` contacts at 240/520 ms, `attack_spin` at 420 ms, and `attack_jump` at 460 ms. The nominal start interval is 1120 ms. One accuracy/critical roll and one damage budget apply to the entire action; cross splits that budget approximately 50/50, with integer rounding remainder on the second contact. Successful basic actions grant 2 mana once, including a two-contact cross; misses grant none.
- Crescent Tempest / Sichelsturm (`hero_kael_ultimate`, animation `skill`) lasts 1320 ms and contacts at 400/920 ms, splitting its total 4× attack budget 25/75. It consumes all 26 mana and starts a 4500 ms cooldown on acceptance (`cooldownUntilMs`). Actual contact damage is capped by the remaining enemy HP. Kael's actions are serialized through their full duration, so the next action cannot interrupt recovery. He has no passive or ultimate healing. Server skills remain automatic; playback controls cannot change the committed result.
- The opt-in `TestKaelPostgresMigrationAndReload` accepts only `MYTHWAKE_KAEL_TEST_DATABASE_URL` pointing to an empty `kael_migration_test_*` database on `127.0.0.1:18434`; it never uses the ordinary database URL. It verifies the 0001–0033 upgrade to 0034, repeat migration, exact catalog rows, preservation of existing player progress, starter grant and persistence after reopening connections. Run with `go test ./internal/database -run TestKaelPostgresMigrationAndReload -v -count=1` against a separate temporary PostgreSQL cluster.
...
- Production-ready account recovery and balance tooling/admin flow
...
- Android local tester APK default backend URL: `http://10.0.2.2:8080` for MuMuPlayer and standard Android emulators; physical-device or server builds should pass an explicit `-BackendBaseUrl` value.
