...
func TestSnapshotIncludesCoreDefinitionSets(t *testing.T) {
	...
	if len(snapshot.Heroes) != 6 {
		t.Fatalf("expected 6 hero definitions, got %#v", snapshot.Heroes)
	}
	...
	if len(snapshot.SummonBanners) != 1 || len(snapshot.SummonBanners[0].ShardDrops) != 6 {
		t.Fatalf("expected standard summon banner with 6 shard drops, got %#v", snapshot.SummonBanners)
	}
