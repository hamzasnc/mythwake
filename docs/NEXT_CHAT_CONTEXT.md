...
Current branch:
- `codex/internal-alpha-candidate`
...
Latest known pushed commit before the 0.2.159 continuation:
- `24b2ab4 Rework battle formation mockup`

Current continuation:
- Prototype `0.2.177` / Backend `0.2.63` adds the server-authoritative Tower Dungeon slice. `POST /dungeons/tower_dungeon/run?floor=N` uses action ID `tower_run`, active-floor validation, combat replay, Gold/Myth Essence/Hero Shard rewards, and idempotent replay. PostgreSQL migration `0032_tower_progression.sql` adds `common.tower_definitions`, `player.player_tower_progress`, and `debug.v_player_tower_overview`; `/definitions` exposes `towers`, snapshots expose `tower`, and Unity routes Server Mode Tower through the existing visible dungeon combat. Backend balance/player/HTTP tests and `go vet` pass. Direct Runtime C# compilation against the installed Unity 6 runtime/module references also passes with 0 errors. Unity Current Slice stopped before project validation because this environment has no valid Unity Editor license; PostgreSQL restart/re-login E2E and Android packaging/device smoke are still owner-run gates.
...
- Future account direction now has an Email + Password MVP: durable tester accounts have backend register/login/logout, Account Start + Backend-panel UI, cached session Continue, and PostgreSQL E2E coverage for restart/re-login persistence. Google Login through Play Store / Google Play Services should come later. Do not build full Google Login yet.
...
- Unity editor validators now cover Village map/build/detail/upgrade/demolish, Dungeons map/zoom clamp/Formation entry and back navigation, Fast Rewards, Mobile UX, Account Start, Summon/Vanguard Oath including result summary/button text fit, Upgrade Clutter, Home Idle Combat, Fight/Formation visible controls, slot/button spacing, result flow, Early Game Loop fresh-save progression, Paladin integration, and Paladin Spine handoff. `Mythwake/Validate Current Slice` runs them together, and `scripts/check-unity-current-slice.cmd` runs the same check in Unity batchmode when no editor instance has the project open.
...
- Home Next Goal now points through the early loop in order: push Campaign when Power is ready, otherwise Gear drops/equip, Weapon/Armor/accessory/Hero upgrades, affordable Village build/upgrade, Gold/Essence Dungeon resource routes, Gear Dungeon drops, Summon shards, or concrete Gold/Essence/Power farm gaps. The Early Game Loop validator checks fresh EN/DE guidance plus a forced resource-gap route.
...
- Redis optional.
- Guest and Email + Password auth issue Bearer sessions that resolve one active player state.
