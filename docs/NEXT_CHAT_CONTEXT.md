...
- Hero Detail now exposes all 2 equipment tracks plus all 6 accessory slots with armory background and equipped-only slot icon art. The localized main gear action shows Open Gear for equipment slots and Equip Gear for accessory slots, empty accessory slots stay visually empty even when bag copies exist, accessory lists put owned copies above empty rows and higher rarity first inside each group with visible copy/tap-to-equip text, the selected equipment/accessory slot list opens instead of immediately leaving for Gear, the equipment list's Open Gear row navigates to the Gear screen, and Remove Gear unequips accessories locally or through Server Mode via `/gear/accessories/unequip`.
