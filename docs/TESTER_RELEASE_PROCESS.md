...
```powershell
.\scripts\build-android.cmd -OutputPath Builds\Android\Mythwake-0.2.177-internal-alpha.apk
```

AAB for Play Console preparation:

```powershell
.\scripts\build-android.cmd -AppBundle -OutputPath Builds\Android\Mythwake-0.2.177-internal-alpha.aab
```
...
```powershell
adb devices -l
adb install -r Builds\Android\Mythwake-0.2.177-internal-alpha.apk
...
- `go test ./...` and `go vet ./...` in `backend`: passed, including Tower success/defeat/idempotency/restart coverage.
- `scripts/check-unity-csharp.ps1`: passed, with existing serialized-field warnings only.
- `scripts/check-unity-current-slice.ps1`: passed, including the Crystal shop tab/content check.
- `git diff --check`: clean for the candidate changes.
- APK/AAB packaging for `0.2.177` is tracked by `docs/TESTER_BUILD_NOTES.md` and `docs/RELEASE_GATES.md`.
