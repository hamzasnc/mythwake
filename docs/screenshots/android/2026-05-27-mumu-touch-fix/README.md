...
- APKs: `Builds/Android/Mythwake-0.2.141-mumu.apk` for the original touch-alignment pass, `Builds/Android/Mythwake-0.2.143-mumu.apk` for the launcher-start regression fix, `Builds/Android/Mythwake-0.2.144-mumu.apk` for the safe-viewport fix, `Builds/Android/Mythwake-0.2.145-mumu.apk` for the Android legacy UI input fix, `Builds/Android/Mythwake-0.2.146-mumu.apk` for the MuMu inverted-mouse-Y correction, and `Builds/Android/Mythwake-0.2.147-mumu.apk` for hard-anchoring the art bottom navigation and applying the pointer-Y correction to the MuMu desktop pointer path even when Android surfaces it as touch. The later 0.2.149 source fix removes the custom pointer transform again.
...
- Follow-up source/build fixes: Prototype `0.2.148` tried X/Y pointer normalization for the later mirrored bottom-nav report. Prototype `0.2.149` superseded that by removing the custom MuMu pointer module, wiring the missing Hero Detail gear slot 8 listener, making the center Campaign/Home nav hit layer raycastable, and adding editor checks for bottom-nav click mappings, button hit targets, and Hero Detail gear-slot mappings. Prototype `0.2.154` removes the later reintroduced X mirror again after Y became correct but X was inverted, keeps Android in generated normal portrait `screenOrientation=1`, uses Unity's standard `InputSystemUIInputModule` without a custom MuMu pointer module, and verifies left Heroes, right Summon, plus Bracelet opening `Bracelet Gear`.
