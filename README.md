...
Prototype version: 0.2.179 internal alpha hero-screen fix
Local save version: 2

Current prototype:
- MuMuPlayer Android builds launch in normal portrait and use Unity's standard Input System UI path without a hand-rolled pointer-coordinate transform.
- Android build profile
- Reproducible Android APK/AAB batch build helper and portrait screenshot fallback helper
- Simple portrait UI
...
- Dungeons have a dedicated map screen with Gold, Essence, Gear, and Tower Trial dungeon cards
...
- Prototype 0.2.170: Prepared the Android tester release process for repeatable multi-tester handoffs. Unity PlayerSettings now use explicit Android identity (`com.xmiepsen.mythwake`, app name `Mythwake`, company `xMiepsen`), Android Version Name matches the visible Prototype version, Android Version Code is derived as `major*1000000 + minor*1000 + patch` (`2170` for `0.2.170`), and Mobile UX validation checks those values. `scripts/build-android.cmd` now supports APK and AAB output (`-AppBundle`) while keeping the Android Build Profile's GUI default as APK. APK `Builds/Android/Mythwake-0.2.170-tester-release.apk` and AAB `Builds/Android/Mythwake-0.2.170-play-internal.aab` build successfully; APK metadata reports package `com.xmiepsen.mythwake`, versionCode `2170`, versionName `0.2.170`, label `Mythwake`, minSdk `25`, targetSdk `36`. MuMu smoke installed/launched the APK, verified Startscreen version, Continue, Guest fallback, Home, Formation/Fight/Result, Heroes, Village, Fast Rewards, Dungeons, Summon, restart/Continue, and filtered Logcat had no Mythwake/Unity crash/ANR/NullReference/missing-asset blocker. Added `docs/TESTER_RELEASE_PROCESS.md` and `docs/TESTER_BUILD_NOTES.md` for build commands, signing boundaries, Play Internal prep, known issues, and feedback fields.
...
- Prototype 0.2.143: Restored the explicit Android `MAIN`/`LAUNCHER` intent filter on `UnityPlayerGameActivity` so MuMuPlayer can open the APK from its launcher/app icon instead of requiring a direct `adb shell am start`.
- Prototype 0.2.142: Removed the fullscreen helper's inset-listener reapply loop after MuMu/GameActivity could repeatedly emit `WINDOW_INSETS_CHANGED` during launch. The fullscreen helper now keeps bounded retries and focus/system-UI reapply without recursively scheduling on every inset callback.
- Prototype 0.2.141: Fixed the MuMuPlayer pointer-coordinate drift by forcing Android GameActivity immersive/fullscreen behavior through a manifest/theme override, a native `MythwakeFullscreen` helper that re-applies fullscreen after launch/resume/focus/inset changes, and full viewport rendering via `androidRenderOutsideSafeArea`. Built and installed `Builds\Android\Mythwake-0.2.141-mumu.apk` in MuMuPlayer (`emulator-5554`, Android 12, 1080x1920, 280 dpi), confirmed `ITYPE_STATUS_BAR visible=false`, cold launch `TotalTime 795 ms`, and verified visible-coordinate taps open Village, Summon, and Battle Formation. Evidence is under `docs/screenshots/android/2026-05-27-mumu-touch-fix/`.
- Prototype 0.2.139: Hardened Android runtime UI input by ensuring a runtime EventSystem with `InputSystemUIInputModule`, disabling incompatible legacy modules under the new Input System, ensuring runtime canvases have GraphicRaycasters, and making generated TMP labels non-raycast by default. Added a small non-blocking runtime FPS overlay, tightened Summon result spacing, Formation density/enemy preview placement, Hero Detail gear-slot side margins, and Result Continue cleanup. Fast Rewards now has a modal touch blocker so Battle/Home controls cannot be clicked through the popup. Built and installed `Builds\Android\Mythwake-0.2.139-mumu.apk` in MuMuPlayer (`emulator-5554`, Android 12, 1080x1920, 280 dpi), measured cold `am start -W` launch at `881 ms` / host stopwatch about `0.92s`, captured real screenshots under `docs/screenshots/android/2026-05-26-touch-polish-final/`, observed `29-30 FPS`, and found no Mythwake/Unity exceptions in filtered Logcat. A physical Android notch/gesture safe-area pass remains open.
- Prototype 0.2.138 MuMu pass: Built `Builds/Android/Mythwake-0.2.138-mumu.apk`, installed and launched it in MuMuPlayer (`emulator-5554`, Android 12, 1080x1920, 280 dpi), measured a cold `am start -W` launch at about 700 ms, captured real screenshots for Home, Stage Detail, Patrol Info, Village, Fast Rewards, Hero Detail, Gear, Summon, Formation, Fight, and Result, recorded filtered Logcat/memory/performance notes without Mythwake/Unity exceptions, fixed the `EquipmentIcons.meta` YAML warning, and hardened the Current Slice wrapper against Unity's benign NamedPipe shutdown IOException.
...
- Prototype 0.2.117: Added matching Home campaign stage-detail state tint, an OK/ZIEL/LOCK badge, and validator coverage for cleared/current/locked detail popups.
- Prototype 0.2.116: Added Home campaign stage-preview state tint and a non-intercepting accent strip for cleared/current/locked selections, with validator coverage.
- Prototype 0.2.115: Added visible ZIEL badges to the current Home campaign node and validator coverage that cleared/locked nodes do not inherit the current target state.
- Prototype 0.2.114: Added visible LOCK badges to locked Home campaign nodes and validator coverage that cleared/current nodes do not inherit the locked state.
- Prototype 0.2.113: Added visible OK badges to cleared Home campaign nodes and validator coverage that current/locked nodes do not inherit the cleared state.
- Prototype 0.2.112: Added a separate selected-stage halo to Home campaign nodes so tapped checkpoints stay distinct from true current-stage progress, with validator coverage for locked selections.
- Prototype 0.2.111: Clarified Home campaign stage-detail reward labels for normal, bonus, and boss stages, with validator coverage for all three reward rows.
- Prototype 0.2.110: Mirrored Boss/Bonus/Normal tags and special reward hints into the Home campaign stage-detail popup, with validator coverage for detail copy fit.
- Prototype 0.2.109: Added Boss/Bonus/Normal tags and reward hints to the Home campaign stage preview, with validator coverage for preview copy fit.
- Prototype 0.2.108: Added visible Bonus badges to non-boss Home campaign milestone nodes and validator coverage for milestone/boss badge separation.
- Prototype 0.2.107: Added visible Boss badges to milestone Home campaign nodes and validator coverage for boss/non-boss node badge state.
- Prototype 0.2.106: Added progress coloring to Home campaign map path segments and validator coverage that locked future paths stay dim.
- Prototype 0.2.105: Added a visible current-stage halo on the Home campaign map and validator coverage so locked nodes cannot show the current marker.
- Prototype 0.2.104: Synced the lower Home idle combat map crop to current stage progress and added validator coverage for Home progress-map UV sync.
...
- Prototype 0.2.90: Extended Village and Fast Rewards validators for scrollable map coverage, loaded building art, close flows, popup exclusivity, and disabled Server Mode fallback claims.
- Prototype 0.2.89: Extended Home idle validation for locked checkpoint detail Battle guards and campaign clear reward payloads.
- Prototype 0.2.88: Guarded the Home stage-detail Battle action and extended validation so the current checkpoint opens the detail popup and enters Formation cleanly.
...
- Prototype 0.2.80: Pulled the Home campaign map up to the top edge under the resource bar and enlarged the lower idle patrol heroes and monsters.
